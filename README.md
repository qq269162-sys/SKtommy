...
- 内嵌一年日频数据(300 个交易日),3M / 6M / 1Y 切换
- 「你付出 vs 你得到」对照条 + 可改股价的即时计算器
- 简化法 c 与官方 NAV 折价双线对照,可 hover 看每日明细
- 附「数据源校验」面板,列出行情源与官方 NAV 基准不一致的全部 10 天

行情来自 AlphaSquare(`api.alphasquare.co.kr/data/v3/prices/candles`,stock_id 1456 / 3493)。
数据是发布时快照;要看最新数字用上面的 CLI,或在页面里手动填入两个股价。
